...
//    fragColor = color;
    ...
    float attn = lightIntensity * min(1 / (attConstant + attLinear * d + attQuadratic * pow(d, 2)), 1);
    fragColor = color*ambientIntensity + color *attn * lightColor * diffuseIntensity * max(0, dot(CameraSpace_normal_n, CameraSpace_toLight_n));

    // Specular could be wrong.
    vec3 r = reflect(-CameraSpace_toLight_n, CameraSpace_normal_n);
    vec3 spec = color*attn* lightColor * specularIntensity * pow(max(0, dot(CameraSpace_toEye_n, r)), shininess);
    fragColor += spec;

    fragColor = vec4(vec3(0.1f, 0.2f, 0.3f), 1.0);
}
